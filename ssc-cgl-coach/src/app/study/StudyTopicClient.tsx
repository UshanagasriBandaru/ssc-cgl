...
type AnalysisResult = {
  title: string; keyPoints: string[]; formulas: string[];
  sscRelevance: string; shortcuts: string[]; potentialQuestions: string[];
};
...
type AnalysisTab = "points" | "formulas" | "shortcuts" | "questions";
...
export function StudyTopicClient() {
  ...
  const [analysisTab, setAnalysisTab] = useState<AnalysisTab>("points");
  ...
  async function generateFor(name: string) {
    ...
    } catch { setErr("Network error."); setPack(null); }
    finally { setBusy(false); }
  }

  function handleSavePack() {
    if (!pack) return;
    setSavedPacks(persistPack(pack));
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-1 rounded-2xl border border-zinc-200 bg-zinc-100 p-1 dark:border-zinc-800 dark:bg-zinc-900">
        {([
          { id: "generate" as MainTab, label: "Generate notes" },
          { id: "source" as MainTab, label: "Analyse source" },
          { id: "saved" as MainTab, label: `Saved (${savedPacks.length})` },
        ]).map((t) => (
          <button key={t.id} type="button" onClick={() => setMainTab(t.id)}
            className={`flex flex-1 items-center justify-center rounded-xl px-3 py-2.5 text-sm font-semibold transition ${mainTab === t.id ? "bg-white text-zinc-900 shadow-sm dark:bg-zinc-800 dark:text-zinc-50" : "text-zinc-500 hover:text-zinc-700 dark:text-zinc-400"}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <div className="mt-2 flex gap-2">
              <input className="flex-1 rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm placeholder-zinc-400 shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-50"
                value={input} onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && void generateFor(input)}
                placeholder="e.g. Percentage, Time & Work, Indian Polity…" />
              <button type="button" disabled={busy || !input.trim()} onClick={() => void generateFor(input)}
                className="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50 hover:bg-zinc-800 dark:bg-amber-500 dark:text-zinc-900">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {QUICK_TOPICS.map((t) => (
                <button key={t} type="button" onClick={() => { setInput(t); void generateFor(t); }}
                  className="rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-0.5 text-xs text-zinc-600 hover:border-zinc-400 hover:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">
                  {t}
                </button>
              ))}
            </div>
            {err && <div className="mt-3 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300"><span>Warning</span><span>{err}</span></div>}
          </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Link href={mockHref} className="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-800 dark:bg-amber-500 dark:text-zinc-900">
                    Mock test
                  </Link>
                </div>
              </div>
              ...
              <div className="flex gap-1 rounded-xl border border-zinc-200 bg-zinc-100 p-1 dark:border-zinc-800 dark:bg-zinc-900">
                {([
                  { id: "notes" as PackTab, label: "Notes" },
                  { id: "formulas" as PackTab, label: "Formulas" },
                  { id: "shortcuts" as PackTab, label: `Shortcuts (${pack.shortcuts.length})` },
                ]).map((t) => (
                  <button key={t.id} type="button" onClick={() => setPackTab(t.id)}
                    className={`flex flex-1 items-center justify-center rounded-lg px-3 py-2 text-sm font-medium transition ${packTab === t.id ? "bg-white text-zinc-900 shadow-sm dark:bg-zinc-800 dark:text-zinc-50" : "text-zinc-500 hover:text-zinc-700 dark:text-zinc-400"}`}>
                    {t.label}
                  </button>
                ))}
              </div>
          ...
          {!pack && !busy && (
            <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/50 p-10 text-center dark:border-zinc-700 dark:bg-zinc-900/20">
              <p className="font-medium text-zinc-700 dark:text-zinc-300">Pick a topic above to get started</p>
      ...
      {mainTab === "source" && (
        <div className="space-y-5">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <p className="mt-1 text-sm text-zinc-500">Paste text from any book, article, or notes. AI extracts key points, formulas, shortcuts, and likely SSC MCQ questions.</p>
            <div className="mt-4 space-y-3">
              <label className="block text-sm">
                ...
                <input className="mt-1.5 w-full rounded-xl border border-zinc-300 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-50"
                  ...
                  placeholder="e.g. Indian Polity, Percentage…" />
              </label>
              <label className="block text-sm">
                <span className="font-medium text-zinc-700 dark:text-zinc-300">Source text ({sourceText.length} / 15,000 chars)</span>
                <textarea className="mt-1.5 min-h-[200px] w-full rounded-xl border border-zinc-300 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-50"
                  value={sourceText} onChange={(e) => setSourceText(e.target.value)}
                  placeholder="Paste any text — Lucent excerpt, NCERT paragraph, newspaper article, your notes, PYQ solution…" />
              </label>
              <div className="flex flex-wrap gap-2">
                <button type="button" disabled={sourceBusy || sourceText.trim().length < 30} onClick={() => void analyseSource()}
                  className="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50 hover:bg-zinc-800 dark:bg-amber-500 dark:text-zinc-900">
                  {sourceBusy ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white" />Analysing…</> : "Extract key points"}
                </button>
                {sourceText && (
              ...
              {sourceErr && <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300"><span>Warning</span><span>{sourceErr}</span></div>}
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900/60 dark:bg-emerald-950/20">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">{analysis.keyPoints.length} key points</span>
                </div>
              </div>

              <div className="flex gap-1 rounded-xl border border-zinc-200 bg-zinc-100 p-1 dark:border-zinc-800 dark:bg-zinc-900">
                {([
                  { id: "points" as AnalysisTab, label: `Key Points (${analysis.keyPoints.length})` },
                  { id: "formulas" as AnalysisTab, label: `Formulas (${analysis.formulas.length})` },
                  { id: "shortcuts" as AnalysisTab, label: `Shortcuts (${analysis.shortcuts.length})` },
                  { id: "questions" as AnalysisTab, label: `MCQ Stems (${analysis.potentialQuestions.length})` },
                ]).map((t) => (
                  <button key={t.id} type="button" onClick={() => setAnalysisTab(t.id)}
                    className={`flex flex-1 items-center justify-center rounded-lg px-2 py-2 text-xs font-semibold transition sm:text-sm ${analysisTab === t.id ? "bg-white text-zinc-900 shadow-sm dark:bg-zinc-800 dark:text-zinc-50" : "text-zinc-500 hover:text-zinc-700 dark:text-zinc-400"}`}>
                    {t.label}
                  </button>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60">
                ...
                {analysisTab === "formulas" && (
                  analysis.formulas.length
                    ? <ul className="space-y-3">{analysis.formulas.map((f, i) => (<li key={i} className="flex items-start gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">{i + 1}</span><code className="text-sm text-zinc-800 dark:text-zinc-200">{f}</code></li>))}</ul>
                    : <p className="text-sm text-zinc-500">No formulas found in this source.</p>
                )}
                {analysisTab === "shortcuts" && (
                  analysis.shortcuts.length
                    ? <ul className="space-y-3">{analysis.shortcuts.map((s, i) => (<li key={i} className="flex items-start gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">{i + 1}</span><span className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{s}</span></li>))}</ul>
      ...
      {mainTab === "saved" && (
        <div className="space-y-4">
          {savedPacks.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/50 p-10 text-center dark:border-zinc-700 dark:bg-zinc-900/20">
              <p className="font-medium text-zinc-700 dark:text-zinc-300">No saved packs yet</p>
              ...
              {savedPacks.map((p) => (
                <li key={p.topicName} className="flex flex-col rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/60">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button type="button" onClick={() => setSavedPacks(removeSavedPack(p.topicName))}
                      className="shrink-0 text-xs text-red-400 hover:text-red-600">Remove</button>
                  </div>
